...
class LoggerClassGeneratorAstValidator {
  ...
  static void validateLoggerImports(
      String generatedCode, Set<String> expectedImports) {
    final unit = AstHelper.parseCode(generatedCode);

    // Required framework imports
    expect(AstHelper.hasImport(unit, 'package:flutter/foundation.dart'), isTrue,
        reason: 'Should import Flutter foundation');
    expect(AstHelper.hasImport(unit, 'package:logger/logger.dart'), isTrue,
        reason: 'Should import logger package');

    // Custom imports from config
    for (final importPath in expectedImports) {
      expect(AstHelper.hasImport(unit, importPath), isTrue,
...
class LoggerComponentAstValidator {
  ...
  static void validateLoggerFactory(
    ...
    if (factoryFunction != null) {
      ...
      _validateFactoryOutputs(
          factoryFunction, expectedOutputs, disableReleaseConsoleOutput);
    }
  }
  ...
  static void _validatePrinterConstructor(ClassDeclaration printerClass) {
    final constructor =
        AstHelper.findConstructor(printerClass, 'SimpleLogPrinter');
